...
def index(request):
    productos_destacados = Product.objects.filter(
        product_tags__tag_name='destacado')
...
def producto(request):
    ...
    if tag_filtro:
        productos_filtrados = Product.objects.filter(
            product_tags__tag_name=tag_filtro)
    else:
...
class CustomLoginView(LoginView):
    ...
    def form_invalid(self, form):
        messages.error(
            self.request, 'Usuario o contraseña incorrectos. Por favor, inténtalo de nuevo.')
